...
=== Installation
Copy tabbed_content_helper.rb into RAILS_ROOT/app/helpers.

The typical usage of the helper requires prototype (prototype.js) and, if you opt
to use one of the animation effects, scriptaculous (effects.js).

However, it's possible to use the helper with another javascript library or with
custom code by configuring it to generate only the tab markup. See the :html_only
option for details.

=== Usage

  tabbed_content(tab_container_id = "tabs", options = {}){|tab_builder| block}
